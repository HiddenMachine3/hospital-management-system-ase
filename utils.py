...
def sort_people_list_by_column(treeview, arr, col, reverse):
    ...
    if col == "Date of Birth":
        arr.sort(
            key=lambda x: datetime.datetime.strptime(x.get_dob(), "%d/%m/%Y"),
            reverse=reverse,
        )
    ...
    treeview.heading(
        col,
        text=col,
        command=lambda _col=col: sort_people_list_by_column(
            treeview, arr, _col, not reverse
        ),
    )
...
def sort_medicines_list_by_column(treeview, arr, col, reverse):
    ...
    treeview.heading(
        col,
        text=col,
        command=lambda _col=col: sort_medicines_list_by_column(
            treeview, arr, _col, not reverse
        ),
    )
